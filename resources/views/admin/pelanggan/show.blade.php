    ...
    <div class="w-full max-w-sm">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 py-5 flex items-center">
                ...
                <h1 class="text-lg font-semibold text-white">Detail Pelanggan</h1>
            </div>
            ...
            <div class="p-6 space-y-5">
                <div>
                    ...
                    <p class="mt-1 text-base font-semibold text-gray-900">{{ $pelanggan->nama_lengkap }}</p>
                </div>

                <div>
                    <label class="text-xs font-medium text-gray-600">Email</label>
                    <p class="mt-1 text-base text-gray-900 break-all">{{ $pelanggan->email }}</p>
                </div>

                <div>
                    <label class="text-xs font-medium text-gray-600">Nomor Telepon</label>
                    <p class="mt-1 text-base text-gray-900">{{ $pelanggan->nomor_telepon }}</p>
                </div>

                <div>
                    <label class="text-xs font-medium text-gray-600">Alamat</label>
                    <p class="mt-1 text-base text-gray-900">{{ $pelanggan->alamat ?? '-' }}</p>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <span class="inline-block px-3 py-1.5 rounded-full text-xs font-medium
                            {{ $pelanggan->status == 'aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            {{ ucfirst($pelanggan->status) }}
                        </span>
                    </div>
                </div>

                {{-- Jika pelanggan punya peran (misalnya dari relasi atau kolom), bisa ditampilkan. Jika tidak, hapus saja baris ini --}}
                <div>
                    <label class="text-xs font-medium text-gray-600">Peran</label>
                    <p class="mt-1 text-base text-gray-900 capitalize">{{ $pelanggan->peran ?? 'Pelanggan' }}</p>
                </div>
