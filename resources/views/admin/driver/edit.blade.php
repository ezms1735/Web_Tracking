    ...
    <div class="w-full max-w-sm">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-5 flex items-center">
                ...
                <h1 class="text-lg font-semibold text-white">Edit Driver</h1>
            </div>

            <!-- Form -->
            <div class="p-6">
                <form action="{{ route('admin.driver.update', $driver->id) }}" method="POST" class="space-y-5">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Nama Lengkap</label>
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $driver->nama_lengkap) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="nomor_telepon" value="{{ old('nomor_telepon', $driver->nomor_telepon) }}" required
                               class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                    </div>
